...
pub fn main() !void {
    ...
    connection.executeWithParams("INSERT INTO dns_test (domain, ip) VALUES (?, ?)", .{ "example.com", "127.0.0.1" }) catch |err| {
        std.log.err("Failed to insert data: {}", .{err});
        return;
    };

    const result = connection.queryWithParams("SELECT COUNT(*) as count FROM dns_test WHERE domain = ?", .{"example.com"}) catch |err| {
