...
pub const EnhancedDNSCache = struct {
    ...
    pub fn get(self: *EnhancedDNSCache, cache_key: []const u8) ?dns.DNSPacket {
        ...
        if (self.database.getCachedDNSResponse(domain, query_type, query_class)) |response_data| {
            defer self.allocator.free(response_data);
            
            // Deserialize DNS packet from database
            var stream = std.io.fixedBufferStream(response_data);
            if (dns.DNSPacket.deserialize(self.allocator, stream.reader())) |packet| {
                _ = self.database_hits.fetchAdd(1, .monotonic);
                
                // Add back to memory cache for future fast access
                self.putInMemoryCache(cache_key, packet, 300) catch |err| {
                    log.warn("Failed to cache in memory: {}", .{err});
                };
                
                log.debug("🗄️  Database cache HIT for: {s}", .{cache_key});
                return packet;
            } else |err| {
                log.warn("Failed to deserialize cached packet: {}", .{err});
            }
        } else |err| {
            log.debug("Database lookup error: {}", .{err});
        }
    ...
    pub fn put(
        ...
        log.debug("💽 Cached response for: {s} (TTL: {}s)", .{ cache_key, ttl });
    }
    ...
    fn putInDatabase(
        ...
        try self.database.cacheDNSResponse(
            domain,
            query_type,
            query_class,
            buffer.items,
    ...
    pub fn cleanup(self: *EnhancedDNSCache) !u64 {
        ...
        if (expired_count > 0) {
            log.info("🧹 Cleaned up {} expired cache entries", .{expired_count});
        }
...
pub const CacheStats = struct {
    ...
    pub fn format(
        ...
        _ = options;
        try writer.print(
            "Cache Stats: Total={}, Memory={}, DB={}, Misses={}, Hit Rate={d:.1}%",
            .{ self.total_queries, self.memory_hits, self.database_hits, self.misses, self.overall_hit_rate * 100 }
        );
    }
};
