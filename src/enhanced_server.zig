//! Enhanced CNS Server with HTTP/3, QUIC, TLS 1.3, and ZQLite v0.7.0 support
//! Leverages Shroud unified networking library and ZQLite database
...
pub const EnhancedServer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config_path: ?[]const u8) !EnhancedServer {
        ...
        const enhanced_dns_cache = try enhanced_cache.EnhancedDNSCache.init(allocator, db, cfg.cache_size / 4 // Keep 1/4 in memory, rest in database
        );
    ...
    pub fn start(self: *EnhancedServer) !void {
        ...
        log.info("📊 Cache: {d} memory entries, persistent database backend", .{self.config.cache_size / 4});
